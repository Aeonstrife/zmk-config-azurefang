...
static int led_profile_listener_cb(const zmk_event_t *eh) {
    ...
    struct blink_item blink = {.duration_ms = CONFIG_AZUREFANG_OUTPUT_BLINK_MS};
...
static int led_battery_listener_cb(const zmk_event_t *eh) {
    ...
    if (battery_level <= CONFIG_AZUREFANG_BATTERY_LEVEL_CRITICAL) {
        LOG_INF("Battery level %d, blinking red for critical", battery_level);

        struct blink_item blink = {.duration_ms = CONFIG_AZUREFANG_BATTERY_BLINK_MS,
                                   .color = LED_RED};
        k_msgq_put(&led_msgq, &blink, K_NO_WAIT);
    }
    return 0;
...
extern void led_thread(void *d0, void *d1, void *d2) {
    ...
    if (battery_level >= CONFIG_AZUREFANG_BATTERY_LEVEL_HIGH) {
        LOG_INF("Battery level %d, blinking green", battery_level);
        blink.color = LED_GREEN;
    } else if (battery_level >= CONFIG_AZUREFANG_BATTERY_LEVEL_LOW) {
    ...
    while (true) {
        ...
        k_sleep(K_MSEC(CONFIG_AZUREFANG_INTERVAL_MS));
    }
}
